...
import * as ko from 'blockly/msg/ko'; // 한글 메시지를 가져옴
            ...
            if (data && data.type && data.type.endsWith('.xml')) {
                ...
                fetch('/read-file', {
                    ...
                    .then(result => {
						...
						// 블록이 워크스페이스에 적용된 후, 해당 블록을 삭제
                        const blockToDelete = demoWorkspace.getBlockById(primaryEvent.blockId);
                        if (blockToDelete) {
                            blockToDelete.dispose();  // 블록을 삭제
                            console.log("Block has been deleted after adding XML content.");
                        } else {
                            console.error("Block to delete not found.");
                        }
